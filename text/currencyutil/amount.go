...
func NewAmountFloat(u string, v float64) (Amount, error) {
	...
	can, err := currencyutil.ParseCurrencyUnit(u, "")
	if err != nil {
		return amt, err
	}
	amt.Unit = can
...
func ParseAmount(value string) (Amount, error) {
	...
	if len(m) > 0 {
		...
		pr.Value = n
		cur, err := currencyutil.ParseCurrencyUnit(m[1], m[2])
		if err != nil {
			return pr, err
		}
		pr.Unit = cur
...
func (amt Amount) MustStringFixed(places int32, defaultUnit string) string {
	...
	if places < 1 {
		return currencyutil.MustSymbol(amt.Unit) + is
	}
	...
	return currencyutil.MustSymbol(unit) + is + exp
}
